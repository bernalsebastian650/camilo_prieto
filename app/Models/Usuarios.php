...
class Usuarios extends AbstractDBConnection implements Model
{
    ...
    {
        $query = "INSERT INTO doblea.usuarios VALUES (
    ...
    {
        $query = "UPDATE doblea.usuarios SET 
    ...
    public static function searchForId(int $id): ?Usuarios
    {
        try {
            if ($id > 0) {
                ...
                $tmpUsuario->Connect();
                $getrow = $tmpUsuario->getRow("SELECT * FROM doblea.usuarios WHERE id =?", array($id));
    ...
    {
        return Usuarios::search("SELECT * FROM doblea.usuarios");
    }
    ...
    {
        $result = Usuarios::search("SELECT * FROM doblea.usuarios where documento = " . $documento);
