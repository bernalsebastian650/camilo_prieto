...
class Compras extends AbstractDBConnection implements Model
{
    ...
    public function getDetalleCompra(): ?array
    {
        ...
        $this->detalleCompra = DetalleCompras::search('SELECT * FROM doblea.detalle_compras where compra_id = '.$this->id);
    ...
    {
        $query = "INSERT INTO doblea.compras VALUES (:id,:numero_serie,:empleado_id,:proveedor_id,:fecha_compra,:monto,:estado,:created_at,:updated_at)";
    ...
    {
        $query = "UPDATE doblea.compras SET 
    ...
    public static function searchForId($id) : ?Compras
    {
        try {
            if ($id > 0) {
                $Compra = new Compras();
                $Compra->Connect();
                $getrow = $Compra->getRow("SELECT * FROM doblea.compras WHERE id =?", array($id));
                $Compra->Disconnect();
    ...
    {
        return Compras::search("SELECT * FROM doblea.compras");
    }
    ...
    public static function facturaRegistrada($numeroSerie): bool
    {
        ...
        $result = Compras::search("SELECT id FROM doblea.compras where numero_serie = '" . $numeroSerie. "'");
