...
class Categorias extends AbstractDBConnection implements Model
{
    ...
    {
        $this->productosCategoria = Productos::search("SELECT * FROM doblea.productos WHERE categoria_id = ".$this->id." and estado = 'Activo'");
    ...
    {
        $query = "INSERT INTO doblea.categorias VALUES (:id,:nombre,:descripcion,:estado,:created_at,:updated_at)";
    ...
    {
        $query = "UPDATE doblea.categorias SET 
    ...
    public static function searchForId($id) : ?Categorias
    {
        try {
            if ($id > 0) {
                ...
                $Categoria->Connect();
                $getrow = $Categoria->getRow("SELECT * FROM doblea.categorias WHERE id =?", array($id));
    ...
    {
        return Categorias::search("SELECT * FROM doblea.categorias");
    }
    ...
    public static function categoriaRegistrada($nombre): bool
    {
        ...
        $result = Categorias::search("SELECT id FROM doblea.categorias where nombre = '" . $nombre. "'");
