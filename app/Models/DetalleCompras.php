...
class DetalleCompras extends AbstractDBConnection implements Model
{
    ...
    {
        $query = "INSERT INTO doblea.detalle_compras VALUES (:id,:producto_id,:compra_id,:cantidad,:precio_venta,:created_at)";
        if($this->save($query)){
    ...
    {
        $query = "UPDATE doblea.detalle_compras SET 
    ...
    public static function searchForId($id) : ?DetalleCompras
    {
        try {
            if ($id > 0) {
                ...
                $getrow = $DetalleCompra->getRow("SELECT * FROM doblea.detalle_compras WHERE id = ?", array($id));
    ...
    {
        return DetalleCompras::search("SELECT * FROM doblea.detalle_compras");
    }
    ...
    {
        $result = DetalleCompras::search("SELECT id FROM doblea.detalle_compras where compra_id = '" . $compra_id. "' and producto_id = '" . $producto_id. "'");
        if (count($result) > 0) {
            return true;
        } else {
            return false;
