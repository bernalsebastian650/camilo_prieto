...
class Ventas extends AbstractDBConnection implements Model
{
    ...
    public function getDetalleVenta(): ?array
    {
        ...
        $this->detalleVenta = DetalleCompras::search('SELECT * FROM doblea.detalle_ventas where venta_id = '.$this->id);
    ...
    {
        $query = "INSERT INTO doblea.ventas VALUES (:id,:numero_serie,:cliente_id,:empleado_id,:fecha_venta,:monto,:estado,:created_at,:updated_at)";
    ...
    {
        $query = "UPDATE doblea.ventas SET 
    ...
    public static function searchForId($id) : ?Ventas
    {
        try {
            if ($id > 0) {
                $Venta = new Ventas();
                $Venta->Connect();
                $getrow = $Venta->getRow("SELECT * FROM doblea.ventas WHERE id =?", array($id));
                $Venta->Disconnect();
    ...
    {
        return Ventas::search("SELECT * FROM doblea.ventas");
    }
    ...
    public static function facturaRegistrada($numeroSerie): bool
    {
        ...
        $result = Compras::search("SELECT id FROM doblea.ventas where numero_serie = '" . $numeroSerie. "'");
