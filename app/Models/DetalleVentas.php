...
class DetalleVentas extends AbstractDBConnection implements Model
{
    ...
    {
        $query = "INSERT INTO doblea.detalle_ventas VALUES (:id,:venta_id,:producto_id,:cantidad,:precio_venta,:created_at)";
        if($this->save($query)){
    ...
    {
        $query = "UPDATE doblea.detalle_ventas SET 
    ...
    public static function searchForId($id) : ?DetalleVentas
    {
        try {
            if ($id > 0) {
                ...
                $getrow = $DetalleVenta->getRow("SELECT * FROM doblea.detalle_ventas WHERE id = ?", array($id));
    ...
    {
        return DetalleVentas::search("SELECT * FROM doblea.detalle_ventas");
    }
    ...
    {
        $result = DetalleVentas::search("SELECT id FROM doblea.detalle_ventas where venta_id = '" . $venta_id. "' and producto_id = '" . $producto_id. "'");
        if (count($result) > 0) {
            return true;
        } else {
            return false;
