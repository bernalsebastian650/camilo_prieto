...
class Fotos extends AbstractDBConnection implements Model
{
    ...
    {
        $query = "INSERT INTO doblea.fotos VALUES (:id, :nombre, :descripcion, :producto_id, :ruta, :estado, :created_at, :updated_at)";
    ...
    {
        return Fotos::search("SELECT * FROM doblea.fotos");
    }
    ...
    static function searchForId(int $id): ?object
    {
        try {
            if ($id > 0) {
                $Foto = new Fotos();
                $Foto->Connect();
                $getrow = $Foto->getRow("SELECT * FROM doblea.fotos WHERE id =?", array($id));
                $Foto->Disconnect();
