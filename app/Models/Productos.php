...
class Productos extends AbstractDBConnection implements Model
{
    ...
    {
        $this->fotosProducto = Fotos::search("SELECT * FROM doblea.fotos WHERE producto_id = ".$this->id." and estado = 'Activo'");
    ...
    {
        $query = "INSERT INTO doblea.productos VALUES (:id,:nombre,:precio,:porcentaje_ganancia,:stock,:categoria_id,:estado,:created_at,:updated_at)";
    ...
    {
        $query = "UPDATE doblea.productos SET 
    ...
    public static function searchForId($id) : ?Productos
    {
        try {
            if ($id > 0) {
                ...
                $Producto->Connect();
                $getrow = $Producto->getRow("SELECT * FROM doblea.productos WHERE id =?", array($id));
                $Producto->Disconnect();
    ...
    {
        return Productos::search("SELECT * FROM doblea.productos");
    }
    ...
    public static function productoRegistrado($nombre): bool
    {
        ...
        $result = Productos::search("SELECT id FROM doblea.productos where nombre = '" . $nombre. "'");
